		...
		template<typename ScalarT, typename... TagsT>
			class LilSparseMatrix
			{
						...
						m_num_cols(val[0].size())
				{
					...
					LilSparseMatrix<ScalarT> &operator=(LilSparseMatrix<ScalarT> const &rhs)
					{
						if (this != &rhs)
						{
							...
							if ((m_num_rows != rhs.m_num_rows) ||
									(m_num_cols != rhs.m_num_cols))
							{
								std::cerr<<"throw DimensionException()";
							}
					...
					template<typename RAIteratorI,
						...
						typename DupT>
									...
									DupT         dup)
							{
								...
								std::cout<<std::endl<<"Debug : 1 Build method from LilSparse Started";
								int nodelets, ele_per_nodelet;

								std::cout<<std::endl<<"Debug : 2 NODELETS :"<<NODELETS()<<std::endl;
								if(m_num_rows < 8)
								{
									nodelets = m_num_rows;
									ele_per_nodelet = 1;
								...
								else
								{
									...
									{
										nodelets = 8;
										ele_per_nodelet = m_num_rows / 8;
									}		
									else // load balancing case is pending (if the elements to distribute % 8 != 0)
									{
										nodelets = 8;
										ele_per_nodelet = m_num_rows / 8;
									}		
								...
								pmatrixRow **denseBlockMatrix = (pmatrixRow **) mw_malloc2d(nodelets, ele_per_nodelet * sizeof(pmatrixRow));
								mw_replicated_init((long *)&r_var, (long)denseBlockMatrix);
								...
								{
									for( int j = 0; j<ele_per_nodelet; j++)
									{
									std::cout<<std::endl<<"Nodelet["<<i<<']'<<"["<<j<<"] "<<std::endl;
									for (auto it = r_var[i][j]->begin(); it != r_var[i][j]->end(); ++it)
			 							 {
					...
					template <typename BinaryOpT>
								...
								BinaryOpT merge)
						{
							...
							struct nMapper x = nodeletMapper( irow);

							std::cout<<std::endl<<"irow "<<irow<<'\t'<<"icol "<<icol<<'\t'<<"val "<<val<<'\t'<<"Nodelet num: "<<x.i<<"\t"<<"ele_per_nodelet "<<x.j<<std::endl;

							pmatrixRow *lvar = r_var[x.i];
							r_var[x.i][x.j] = (pmatrixRow)mw_localmalloc(sizeof(matrixRow), r_var[x.i]);
							pmatrixRow dummyPtr = new(lvar[x.j]) matrixRow(); 

							if (r_var[x.i][x.j]->empty())
							{
								r_var[x.i][x.j]->push_back(std::make_tuple(icol, val));
								m_nvals = m_nvals + 1;
							}
							else
							{
								typename std::vector<std::tuple<IndexType, ScalarT>>::iterator it;
								for (it = r_var[x.i][x.j]->begin(); it != r_var[x.i][x.j]->end(); it++)
								{
									if (std::get<0>(*it) == icol)
									{
									...
									{
										r_var[x.i][x.j]->insert(it, std::make_tuple(icol, val));
										m_nvals = m_nvals + 1;
										return;
									}
								}
					...
					struct nMapper nodeletMapper(IndexType rowNumber)
					{	
						struct nMapper x;
						if ( rowNumber < 8 )
						{
				...
				private:
					...
					typedef std::vector<std::tuple<IndexType, ScalarT>>* pmatrixRow;
					typedef std::vector<std::tuple<IndexType, ScalarT>> matrixRow;
					replicated pmatrixRow **r_var;
